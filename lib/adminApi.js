...
const ADMIN_CAR_IMAGES_PROXY_PATH = "/api/admin/car-images";
const ADMIN_CAR_INSTANCE_IMAGES_PROXY_PATH = "/api/admin/car-instance-images";
const ADMIN_INSTANCES_PROXY_PATH = "/api/admin/car-instances";
const ADMIN_CAR_PRICES_PROXY_PATH = "/api/admin/car-prices";
const ADMIN_RESERVATIONS_PROXY_PATH = "/api/admin/car-reservations";
const ADMIN_AVAILABLE_VEHICLES_PROXY_PATH = "/api/admin/available-vehicles";
const ADMIN_BLOGS_PROXY_PATH = "/api/admin/blogs";
...
export async function adminGetCars({ limit, page, signal } = {}) {
  ...
  if (limit !== undefined) {
    ...
    params.set("itemsPerPage", limitValue);
...
export async function adminGetCarInstance(registrationNumber, { signal } = {}) {
  if (!registrationNumber) {
    throw new Error("Registration number is required.");
  }
  const headers = withAuthHeaders();
  const response = await fetch(
    `${ADMIN_INSTANCES_PROXY_PATH}/${encodeURIComponent(registrationNumber)}`,
    {
      method: "GET",
      signal,
      cache: "no-store",
  ...
  if (!response.ok) {
    const message =
      payload?.message ||
      payload?.error ||
      "Neuspješno učitavanje instance vozila.";
    const error = new Error(message);
    error.status = response.status;
    error.payload = payload;
    throw error;
...
export async function adminDeleteCarImage(imageId, { signal } = {}) {
  if (!imageId) {
    throw new Error("Image ID is required.");
  }
  const headers = withAuthHeaders();
  const response = await fetch(
    `${ADMIN_CAR_IMAGES_PROXY_PATH}/${encodeURIComponent(imageId)}`,
    {
      method: "DELETE",
      signal,
      cache: "no-store",
  ...
  if (!response.ok) {
    const message =
      json?.message || json?.error || "Neuspješno brisanje slike.";
    const error = new Error(message);
    error.status = response.status;
    error.payload = json;
    throw error;
...
export async function adminDeleteCarInstanceImage(imageId, { signal } = {}) {
  if (!imageId) {
    throw new Error("Image ID is required.");
  }
  const headers = withAuthHeaders();
  const response = await fetch(
    `${ADMIN_CAR_INSTANCE_IMAGES_PROXY_PATH}/${encodeURIComponent(imageId)}`,
    {
      method: "DELETE",
      signal,
      cache: "no-store",
  ...
  if (!response.ok) {
    const message =
      json?.message || json?.error || "Neuspješno brisanje slike instance.";
    const error = new Error(message);
    error.status = response.status;
    error.payload = json;
    throw error;
...
export async function adminActivateCarInstance(
  registrationNumber,
  { signal } = {}
) {
  if (!registrationNumber) {
  ...
  if (!response.ok) {
    const message =
      json?.message || json?.error || "Neuspješna aktivacija varijacije.";
    const error = new Error(message);
    error.status = response.status;
    throw error;
  }
...
export async function adminGetReservation(id, { signal } = {}) {
  if (!id) {
    throw new Error("Reservation ID is required.");
  }
  const headers = withAuthHeaders();
  const response = await fetch(
    `${ADMIN_RESERVATIONS_PROXY_PATH}/${encodeURIComponent(id)}?_t=${Date.now()}`,
    {
      method: "GET",
      signal,
      cache: "no-store",
  ...
  if (!response.ok) {
    const message =
      payload?.message || payload?.error || "Neuspješno učitavanje rezervacije.";
    const error = new Error(message);
    error.status = response.status;
    error.payload = payload;
    throw error;
...
export async function adminCreateReservation(payload, { signal } = {}) {
  const headers = withAuthHeaders({
    "Content-Type": "application/json",
  });
  const response = await fetch(ADMIN_RESERVATIONS_PROXY_PATH, {
    method: "POST",
    signal,
    cache: "no-store",
    headers,
  ...
  if (!response.ok) {
    const message =
      json?.messageCode || json?.message || json?.error || "Neuspješno kreiranje rezervacije.";
    const error = new Error(message);
    error.status = response.status;
    error.payload = json;
    throw error;
...
export async function adminGetAvailableVehicles({
  carId,
  reservationId,
  startingDateTime,
  endingDateTime,
  ...
  if (reservationId) params.set("reservationId", String(reservationId));
  if (startingDateTime) params.set("startingDateTime", startingDateTime);
  if (endingDateTime) params.set("endingDateTime", endingDateTime);
  const query = params.toString();
  const targetUrl = `${ADMIN_AVAILABLE_VEHICLES_PROXY_PATH}${query ? `?${query}` : ""}`;
  const headers = withAuthHeaders();
  const response = await fetch(targetUrl, {
    method: "GET",
    signal,
  ...
  if (!response.ok) {
    const message =
      payload?.message || payload?.error || "Neuspješno učitavanje dostupnih vozila.";
    const error = new Error(message);
    error.status = response.status;
    throw error;
  }
...
export async function adminUpdateReservation(id, payload, { signal } = {}) {
  if (!id) {
    throw new Error("Reservation ID is required.");
  }
  const headers = withAuthHeaders({
  ...
  const response = await fetch(
    `${ADMIN_RESERVATIONS_PROXY_PATH}/${encodeURIComponent(id)}`,
    {
      method: "PUT",
      signal,
      cache: "no-store",
  ...
  if (!response.ok) {
    const message =
      json?.messageCode || json?.message || json?.error || "Neuspješno ažuriranje rezervacije.";
    const error = new Error(message);
    error.status = response.status;
    error.payload = json;
    throw error;
...
export async function adminGetReservations({
  ...
  const query = params.toString();
  const targetUrl = `${ADMIN_RESERVATIONS_PROXY_PATH}${query ? `?${query}` : ""}`;
  const headers = withAuthHeaders();
  const response = await fetch(targetUrl, {
    method: "GET",
    signal,
  ...
  const payload = await response.json().catch(() => ({
    message: "Podaci o rezervacijama nisu dostupni.",
  }));
  if (!response.ok) {
    const message =
      payload?.message || payload?.error || "Neuspješno učitavanje rezervacija.";
    const error = new Error(message);
    error.status = response.status;
    error.payload = payload;
    throw error;
...
export async function adminGetBlogs({ page, limit, language, isPublished, sort, signal } = {}) {
  const params = new URLSearchParams();
  ...
  const headers = withAuthHeaders();
  const response = await fetch(
    `${ADMIN_BLOGS_PROXY_PATH}${query ? `?${query}` : ""}`,
    { method: "GET", signal, cache: "no-store", headers, credentials: "include" }
  );
  const payload = await response.json().catch(() => null);
  if (!response.ok) {
    const error = new Error(payload?.message || "Neuspješno učitavanje blogova.");
    error.status = response.status;
    throw error;
  }
  return payload;
...
export async function adminGetBlog(id, { signal } = {}) {
  if (!id) throw new Error("Blog ID is required.");
  const headers = withAuthHeaders();
  const response = await fetch(
    `${ADMIN_BLOGS_PROXY_PATH}/${encodeURIComponent(id)}?_t=${Date.now()}`,
    { method: "GET", signal, cache: "no-store", headers, credentials: "include" }
  );
  const payload = await response.json().catch(() => null);
  if (!response.ok) {
    const error = new Error(payload?.message || "Neuspješno učitavanje bloga.");
    error.status = response.status;
    throw error;
  }
  return payload;
...
export async function adminCreateBlog(payload, { signal } = {}) {
  const formData = new FormData();
  ...
  if (payload.metaDescription !== undefined) formData.append("metaDescription", payload.metaDescription);
  if (payload.alias !== undefined) formData.append("alias", payload.alias);
  formData.append("isPublished", payload.isPublished ? "true" : "false");
  formData.append("isMenu", payload.isMenu ? "true" : "false");
  if (isFileLike(payload.coverImage)) formData.append("coverImage", payload.coverImage);

  const headers = withAuthHeaders({});
  const response = await fetch(ADMIN_BLOGS_PROXY_PATH, {
    method: "POST", signal, cache: "no-store", headers, credentials: "include",
    body: formData,
  });
  const json = await response.json().catch(() => null);
  if (!response.ok) {
    const { error } = buildValidationError(json, "Neuspješno kreiranje bloga.");
    error.status = response.status;
    error.payload = json;
    throw error;
  }
...
export async function adminUpdateBlog(id, payload, { signal } = {}) {
  if (!id) throw new Error("Blog ID is required.");
  const formData = new FormData();
  if (payload.title !== undefined) formData.append("title", payload.title);
  if (payload.description !== undefined) formData.append("description", payload.description);
  ...
  if (payload.isMenu !== undefined) formData.append("isMenu", payload.isMenu ? "true" : "false");
  if (isFileLike(payload.coverImage)) formData.append("coverImage", payload.coverImage);

  const headers = withAuthHeaders({});
  const response = await fetch(
    `${ADMIN_BLOGS_PROXY_PATH}/${encodeURIComponent(id)}`,
    { method: "POST", signal, cache: "no-store", headers, credentials: "include", body: formData }
  );
  const json = await response.json().catch(() => null);
  if (!response.ok) {
    const { error } = buildValidationError(json, "Neuspješno ažuriranje bloga.");
    error.status = response.status;
    error.payload = json;
    throw error;
  }
...
export async function adminDeleteBlog(id, { signal } = {}) {
  if (!id) throw new Error("Blog ID is required.");
  const headers = withAuthHeaders();
  const response = await fetch(
    `${ADMIN_BLOGS_PROXY_PATH}/${encodeURIComponent(id)}`,
    { method: "DELETE", signal, cache: "no-store", headers, credentials: "include" }
  );
  if (!response.ok) {
    const json = await response.json().catch(() => null);
    const error = new Error(json?.message || "Neuspješno brisanje bloga.");
    error.status = response.status;
    throw error;
  }
  return true;
