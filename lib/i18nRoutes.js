...
const SEGMENT_TRANSLATIONS = {
  ...
  "blog-list-01": { en: "blog", me: "blog", ru: "blog" },
  "blog-single": { en: "blog", me: "blog", ru: "blog" },
  services: { en: "services", me: "usluge", ru: "uslugi" },
...
export const localizePath = (pathname, locale) => {
  ...
  if (!segments.length) {
    if (PREFIX_DEFAULT_LOCALE || effectiveLocale !== DEFAULT_LOCALE) {
      return ensureTrailingSlash(`/${effectiveLocale}`);
    }
    return "/";
  }
  ...
  return ensureTrailingSlash(localizedPath);
};
