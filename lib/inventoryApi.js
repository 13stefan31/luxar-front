...
const fetchReservationItems = async () => {
  const response = await fetch(`${INVENTORY_API_ROOT}/cars/reservation-items`, {
    headers: getInventoryApiHeaders(),
    cache: "no-store",
  });
  if (!response.ok) {
    throw new Error(`Failed to fetch reservation items: ${response.status}`);
  }
...
const fetchCities = async () => {
  const response = await fetch(`${INVENTORY_API_ROOT}/cities`, {
    headers: getInventoryApiHeaders(),
    cache: "no-store",
  });
  if (!response.ok) {
...
const fetchBlogs = async (language, options = {}) => {
  const params = new URLSearchParams();
  if (language) params.set("language", language);
  if (options.isMenu !== undefined) params.set("isMenu", options.isMenu ? "true" : "false");
  const response = await fetch(
    `${INVENTORY_API_ROOT}/blogs${params.toString() ? `?${params}` : ""}`,
    { headers: getInventoryApiHeaders(), cache: "no-store" }
  );
  if (!response.ok) {
...
const fetchBlog = async (id, language) => {
  const params = new URLSearchParams();
  if (language) params.set("language", language);
  const response = await fetch(
    `${INVENTORY_API_ROOT}/blogs/${encodeURIComponent(id)}${params.toString() ? `?${params}` : ""}`,
    { headers: getInventoryApiHeaders(), cache: "no-store" }
  );
  if (!response.ok) {
